...
class Voedselpakket extends Controller
{
	...
	public function index()
	{
		...
		foreach ($records as $item) {
			...
			$rows .= "<tr>
						...
						<td>
                            <a href='" . URLROOT . "/voedselpakket/update/$item->Id'>wijzig product</a>
                        </td>
						</td>
					</tr>";
}
	...
	public function Getbyid($id){
		...
		$records = $this->voedselpakketModel->getVoedselpakketById($_POST['id']);
		$rows = '';
			foreach ($records as $item) {
				$rows .= "<tr>
				<td>$item->Pakketnummer</td>
				<td>$item->Datumsamenstelling</td>
				<td>$item->DatumUitgifte</td>
				<td>$item->Status</td>
				<td>$item->AantalProductEenheden</td>
				<td></td>
				<td>
				<!-- haalt img van een database op en laat de img zien +clickable link -->
				<a href='" . URLROOT . "/voedselpakket/update/$item->voedselpakketId'>
